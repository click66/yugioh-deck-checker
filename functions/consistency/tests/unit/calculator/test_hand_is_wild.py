...
def test_exact_match():
    ...
    assert hand_is_wild(hand, ideal_hands, card_database, wildcard_lookup) is True


def test_no_match():
    hand = [80181649, 86988864]
    ideal_hands = [[80181649, 14261867]]
    assert hand_is_wild(hand, ideal_hands, card_database, wildcard_lookup) is False
...
def test_wildcard_string_any_spell():
    ...
    hand2 = [86988864, 68170903]  # effect + trap
    assert hand_is_wild(hand2, ideal_hands, card_database, wildcard_lookup) is False


def test_callable_predicate_any_dark():
    is_dark = lambda card_id: card_database[card_id].get("attribute") == "DARK"
...
def test_duplicates_in_hand():
    ...
    assert hand_is_wild(hand, ideal_hands2, card_database, wildcard_lookup) is False
...
def test_empty_hand_or_empty_ideal():
    ...
    hand2 = [80181649]
    ideal_hands2 = []
    assert hand_is_wild(hand2, ideal_hands2, card_database, wildcard_lookup) is False